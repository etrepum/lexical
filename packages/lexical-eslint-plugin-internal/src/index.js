...
import noCrossRealmDom from './rules/no-cross-realm-dom.js';
...
const rules = {
  'no-cross-realm-dom': noCrossRealmDom,
...
// Note: require-pure-annotation and no-cross-realm-dom are deliberately not
// part of the default configs — they only apply to specific file scopes, so
// the root eslint.config.mjs enables each with its own files scope.
const legacyAll = {
  rules: {
