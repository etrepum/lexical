...
/**
 * A generic multi-block editable slot value, adapted from the Lexical
 * playground. It is a shadow root (like a table cell), so a RangeSelection and
 * SELECT_ALL stay scoped to its contents and Backspace at the start is a no-op
 * instead of escaping into the host.
 */
export class SlotContainerNode extends ElementNode {
  $config() {
    return this.config('octane-slot-container', {extends: ElementNode});
  }

  createDOM(): HTMLElement {
    const div = document.createElement('div');
    div.className = 'octane-slot-container';
    return div;
  }
